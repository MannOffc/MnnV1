...
require('./mnn.js')
nocache('../mnn.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./index.js')
nocache('../index.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))

asycn function connectToWhatsapp() {
	const { saveCreds, state } = await useMultiFileAuthState(`session`)
	const auth = useMultiFileAuthState("auth");
	const MannHost = MannHostConnect ({
		printQRInTerminal = true,
		logger: pino({ level: 'silent' }),
		browser ["MnnV1", "Chrome", "1.0.0"],
		auth = auth.state
		})
...
require('./killua')(MannHost, m, chatUpdate, store)
} catch (err) {
console.log(err)}})
...
if (reason === DisconnectReason.badSession) { console.log(`Sorry Your Session Failed, Please Delete And Scan Again`); MannHost.logout(); }
...
} else if (connection === "open") { MannHost.sendMessage(owner + "@s.whatsapp.net", { text: `MnnV1 Started` }); }
